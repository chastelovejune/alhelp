...
class ServiceTeachingController extends AdminBaseController{
  ...
  {
    parent::_initialize();
    $documents= M("service")->where("service_type=2")->order("add_time desc")->select();
    $this->assign("documents",$document);
  }
  public function index()
  {
       $documents= M("service");
    $count      = $documents->where("service_type=2")->count();
    $Page       = new \Think\Page($count,20);
    $show       = $Page->show();// 分页显示输出
    $result = $documents->where("service_type=2")->order("add_time desc")->limit($Page->firstRow.','.$Page->listRows)->select();
 ...
 public function hold($service_id,$reason)
  {//禁用
	$doc['status']=4;
    $documents= M("service")->where("service_type=2 and service_id=$service_id")->save($doc);
    //发送系统消息
	    $desc=M("service")->field("description")->where("service_type=2 and service_id=$service_id")->find();
	    $re=M("service")->field("member_id")->where("service_type=2 and service_id=$service_id")->find();
  ...
  public function top($service_id)
  {
  	...
  	M("service")->where("service_type=2 and service_id=$service_id")->save($data);
  	$this->success("置顶成功",U("index"));
  }
  public function edit($service_id){
  	$re=M("service")->where("service_type=2 and service_id=$service_id")->find();
	  $re=parse_class($re);
  	$this->assign('re',$re);
  	if(IS_POST){
