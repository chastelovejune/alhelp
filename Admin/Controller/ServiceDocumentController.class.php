...
class ServiceDocumentController extends AdminBaseController{
  ...
  {
    parent::_initialize();
    $documents= M("service")->where("service_type=0")->select();
    $this->assign("documents",$document);
  }
  public function index()
  {
    $documents= M("service");
    $count      = $documents->where("service_type=0")->count();
    $Page       = new \Think\Page($count,20);
    $show       = $Page->show();// 分页显示输出
    $result = $documents->where("service_type=0")->order("add_time desc")->limit($Page->firstRow.','.$Page->listRows)->select();
 ...
 // dump($list);
  ...
  {//禁用
	$doc['status']=4;
    $documents= M("service")->where("service_type=0 and service_id=$service_id")->save($doc);
    //发送系统消息
	    $desc=M("service")->field("description")->where("service_type=0 and service_id=$service_id")->find();
	    $re=M("service")->field("member_id")->where("service_type=0 and service_id=$service_id")->find();
  ...
  public function top($service_id)
  {
  	...
  	M("service")->where("service_type=0 and service_id=$service_id")->save($data);
  	$this->success("置顶成功",U("index"));
  }
  public function edit($service_id){
  	$re=M("service")->where("service_type=0 and service_id=$service_id")->find();
	  //查出专业相关start  这里可以优化成JQ加载，以后做
	  /*
	 if($re['unified_id']>0){//查出专业分类 如果是统考
		 $result['type']=M()->query(" select unified.cid from unified left join unified_classify on unified.cid=unified_classify.id where unified.id=".$re['unified_id']);
	  }
	  if($re['school_id']>0){//如果是非统考
		  $result['type']=M()->query("select area_id,school_id,yuan_id,zhuanye_id from zysc_view where zhuanye_id=".$re['school_id']);
	  }

	 $re['type']=$result['type'][0];
	  */
	//  dump($re);
	  $re=parse_class($re);
	  //dump($re);
