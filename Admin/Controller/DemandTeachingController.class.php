...
class DemandTeachingController extends AdminBaseController{
  ...
  {
    parent::_initialize();
    $documents= M("demand")->where("demand_type=2")->order("add_time desc")->select();
    $this->assign("documents",$document);
  }
  public function index()
  {
     $documents= M("demand");
    $count      = $documents->where("demand_type=2")->count();
    $Page       = new \Think\Page($count,20);
    $show       = $Page->show();// 分页显示输出
    $result = $documents->where("demand_type=2")->order("add_time desc")->limit($Page->firstRow.','.$Page->listRows)->select();
 ...
 public function hold($demand_id,$reason)
  ...
  $doc['status']=4;
    $documents= M("demand")->where("demand_type=2 and demand_id=$demand_id")->save($doc);
    //发送系统消息
      $desc=M("demand")->field("description")->where("demand_type=2 and demand_id=$demand_id")->find();
      $re=M("demand")->field("member_id")->where("demand_type=2 and demand_id=$demand_id")->find();
  ...
  public function top($demand_id)
  {
    ...
    M("demand")->where("demand_type=2 and demand_id=$demand_id")->save($data);
    $this->success("置顶成功",U("index"));
  }
  public function edit($demand_id){
    $re=M("demand")->where("demand_type=2 and demand_id=$demand_id")->find();
      $re=parse_class($re);
    $this->assign('re',$re);
    if(IS_POST){
